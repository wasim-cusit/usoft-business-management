...
try {
    $db = getDB();
    $stmt = $db->prepare("SELECT s.*, a.account_name, a.account_name_urdu, a.address, a.city, a.phone, a.mobile FROM sales s 
...
<head>
    ...
    <title><?php echo t('sale_details_title'); ?> - <?php echo htmlspecialchars($sale['sale_no']); ?></title>
...
<body>
    ...
    <div class="bill-info">
        <div class="bill-info-left">
            <div class="info-box">
                ...
                <div class="info-row">
                    ...
                    <span><?php echo htmlspecialchars($sale['sale_no']); ?></span>
                </div>
                <div class="info-row">
        ...
        <div class="bill-info-right">
            <div class="info-box">
                <h3><?php echo t('customer_info'); ?></h3>
                <div class="info-row">
                    <span class="info-label"><?php echo t('customer'); ?>:</span>
                    <span><?php echo displayAccountName($sale); ?></span>
                </div>
                <?php if (!empty($sale['address'])): ?>
                <div class="info-row">
                    <span class="info-label"><?php echo t('address'); ?>:</span>
                    <span><?php echo htmlspecialchars($sale['address']); ?></span>
                </div>
                <?php endif; ?>
                <?php if (!empty($sale['city'])): ?>
                <div class="info-row">
                    <span class="info-label"><?php echo t('city'); ?>:</span>
                    <span><?php echo htmlspecialchars($sale['city']); ?></span>
                </div>
                <?php endif; ?>
                <?php if (!empty($sale['phone']) || !empty($sale['mobile'])): ?>
                <div class="info-row">
                    <span class="info-label"><?php echo t('phone'); ?>:</span>
                    <span><?php echo htmlspecialchars($sale['phone'] ?? $sale['mobile'] ?? '-'); ?></span>
                </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 8%;">توڈا</th>
                <th style="width: 8%;">بھرتی</th>
                <th style="width: 8%;">وزن</th>
                <th style="width: 8%;">کاٹ</th>
                <th style="width: 8%;">صافی</th>
                <th style="width: 10%;"><?php echo t('rate'); ?></th>
                <th style="width: 10%;"><?php echo t('amount'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php if (empty($items)): ?>
        ...
        <tfoot>
            <tr>
                <td colspan="9" style="text-align: <?php echo getDir() == 'rtl' ? 'left' : 'right'; ?>;"><strong><?php echo t('total'); ?>:</strong></td>
    ...
    <div class="total-section">
        <div class="total-row">
            ...
            <span class="total-value"><?php echo formatCurrency($sale['total_amount']); ?></span>
        </div>
        <div class="total-row">
            <span class="total-label"><?php echo t('discount'); ?>:</span>
            <span class="total-value"><?php echo formatCurrency($sale['discount']); ?></span>
        </div>
        <div class="total-row grand-total">
            <span class="total-label"><?php echo t('net_amount'); ?>:</span>
            <span class="total-value"><?php echo formatCurrency($sale['net_amount']); ?></span>
        </div>
        <div class="total-row">
            <span class="total-label"><?php echo t('receipt'); ?>:</span>
            <span class="total-value"><?php echo formatCurrency($sale['paid_amount']); ?></span>
        </div>
        <div class="total-row">
            <span class="total-label"><?php echo t('balance'); ?>:</span>
            <span class="total-value"><?php echo formatCurrency($sale['balance_amount']); ?></span>
        </div>
    </div>
