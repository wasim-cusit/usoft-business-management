...
$pageTitle = 'edit_debit';

$id = intval($_GET['id'] ?? 0);

if (empty($id)) {
    header('Location: ' . BASE_URL . 'transactions/debit.php');
    exit;
}
...
try {
    ...
    $stmt = $db->prepare("SELECT * FROM transactions WHERE id = ? AND transaction_type = 'debit'");
    $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($accountId)) {
        ...
        try {
            ...
            if ($success) {
                header('Location: ' . BASE_URL . 'transactions/debit.php?success=' . urlencode($success));
                exit;
            }
        } catch (PDOException $e) {
...
<div class="page-header">
    <h1><i class="fas fa-edit"></i> <?php echo t('edit'); ?> <?php echo t('debit'); ?></h1>
</div>
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0"><?php echo t('edit'); ?> <?php echo t('debit'); ?> <?php echo t('transaction'); ?></h5>
            </div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <form method="POST" action="">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="<?php echo BASE_URL; ?>transactions/debit.php" class="btn btn-secondary">
